    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Points with animated glassmorphic cards */}
          <motion.ul
          ...
          >
            {step1Data.points.map((point, index) => (
              <motion.li
                key={index}
          ...
          <div className="w-full flex items-center justify-between mt-8">
            {Array.from({ length: 4 }).map((_, idx) => (
              <div key={idx} className="flex-1 h-2 md:h-3 bg-white/20 rounded-full mx-1">
                {idx === 0 && (
                  <motion.div
                    className="h-2 md:h-3 bg-amber-400 rounded-full w-full"
                    animate={{ width: ["0%", "100%"] }}
...
export default Step1AdvancedSlide;
